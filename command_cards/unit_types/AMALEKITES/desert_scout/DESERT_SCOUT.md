# Desert Scout — Command Cards

> **Unit Type:** Light (Fast skirmisher)
> **Faction:** Amalekites
> **Availability:** MVP (enemy)

---

## Formation: DESERT FLANK / DESERT WATCH

### TOP ACTION — Desert Flank

Activations: 2

When an enemy enters line of sight of a Desert Scout,
the Scout attacks immediately before normal initiative order.

Deal 1 additional damage.

**Purpose:**
Ambush and punish careless enemy movement.

---

### BOTTOM ACTION — Desert Watch

Activations: 3

Choose an enemy unit. Reveal: Movement, Attack, HP.

All Desert Scouts gain +1 Attack against that unit this turn.

**Purpose:**
Intelligence gathering that grants a combat bonus.

---

## Unit Stats (from GDD)

| Stat | Value |
|---|---|
| Melee Combat | 1d4 |
| Range | 1 space |
| Move | 4 spaces |
| Discipline | 1 |
| HP | 1 |
| Armor | None |

---

## Notes

- Desert Scouts mirror David's Scout role for the Amalekites.
- Desert Flank is an ambush tool — attacks before initiative.
- Eyes in the Sand is the reconnaissance card — reveals enemy capabilities.
- Amalekite Scouts have the same high mobility (4 spaces) as David's Scouts.