# Slinger — Command Cards

> **Unit Type:** Skirmisher (Ranged)
> **Faction:** Amalekites
> **Availability:** MVP (enemy)

---

## Formation: DESERT STORM / NOMAD GALLOP

### TOP ACTION — Desert Storm

**Activations:** 3

All Amalekite Slingers attack the same enemy.

Gain +1 total Attack for each Slinger in range.

**Purpose:**
Concentrated ranged fire that stacks with numbers.

---

### BOTTOM ACTION — Nomad Gallop

Activations: 2

Slingers may move 2 spaces and attack once.

Movement does not trigger opportunity attacks this turn.

After attacking, Slingers must move at least 1 space further or suffer 1 damage.

**Purpose:**
Mobile ranged harassment that punishes static positioning — stay active or die.

---

## Unit Stats (from GDD)

| Stat | Value |
|---|---|
| Ranged Combat | 1d6 |
| Range | 2 spaces |
| Move | 3 spaces |
| Discipline | 2 |
| HP | 1 |
| Armor | Light |

---

## Notes

- Amalekite Slingers are the same unit type as David's Slingers but with different formation cards.
- Desert Storm is the focused volley — similar to David's Archer formation but with Slinger range.
- Harassment emphasizes the Amalekite hit-and-run identity.