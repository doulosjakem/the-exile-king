# Mighty Men — Formation Cards

> **Unit Type:** Hero (Unique named units)
> **Faction:** David's Company
> **Availability:** Late campaign (post-MVP)

---

## Formation: MIGHTY MEN'S CHARGE / COVENANT BOND

### TOP ACTION — Mighty Men's Charge

**Activations:** 3

When attacking:

- Gain +1 Attack.
- If the attack defeats an enemy,
  move 1 space.

**Purpose:**
Elite warriors breaking enemy lines.

---

### BOTTOM ACTION — Covenant Bond

**Activations:** 2

When a nearby ally would take damage:

- Mighty Men may move adjacent
  and take 1 damage instead.

**Purpose:**
Protect the army's key pieces.

---

## Unit Stats (from GDD)

| Stat | Value |
|---|---|
| Melee Combat | 1d8 |
| Range | Melee only |
| Move | 2 spaces |
| Discipline | 4 |
| HP | 4 |
| Armor | Heavy |
| Notes | Unique named units (Josheb, Eleazar, etc.) |

---

## Notes

- Mighty Men are the elite hero units of David's Company.
- Mighty Men's Charge is the premier offensive card — extra damage plus movement on kill makes them relentless.
- Covenant Bond is the defensive tool — Mighty Men sacrifice themselves to protect weaker units.
- Each Mighty Man has a unique name and may have a unique ability in the full roster.
- These units are only available in the late campaign (post-MVP).